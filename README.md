...
把本地真实资料整理成一套能追溯来源、能人工审核的文字交付包。

**[快速开始](#快速开始)** · [查看使用示例](#使用示例)

**状态：** `v0.1.0 beta`。项目结构与脚本已经实现，实际 Agent 兼容性以[兼容性](#兼容性)中的核验状态为准。

Material to Deliverable 是一个面向学生、职场人和内容工作者的开源 Agent Skill。当你手里有项目文档、会议记录、课程资料、调研文件或反馈记录时，它会先检查材料和任务，再生成带来源索引的汇报、方案、复盘、调研或课程展示。

## 为什么做这个项目

把一批文件直接交给 AI，常见结果是一份很长的摘要。接收者仍然不知道哪些内容是事实、哪些只是意见，关键结论也很难回到原始文件核对。

这个项目只解决一件事：把真实散乱资料整理成可核验的文字交付包。
...
下载完整仓库，将 `material-to-deliverable/` 放入兼容 Agent 配置的 Skills 目录。仓库根目录的 `SKILL.md` 是 Skill 入口。
...
不同客户端也可能使用自己的 Skills 目录。没有实际验证的路径不要当成已支持环境，安装时以对应客户端当前文档为准。
...
Agent 从 Skills 目录加载本项目时，会从 Skill 的安装目录调用同一脚本，不要求用户项目中另有 `scripts/`。
...
Agent 会先检查敏感信息和任务缺口，再逐步建立工作文件。遇到敏感材料、证据不足或未确认的提纲时会暂停。
...
Skill 不修改 `00_input/`。发现敏感信息时只记录风险类别与文件位置，并等待用户确认。主成品完成后会重新读取原始材料审核，验证脚本只检查文件结构和明显残留，不判断事实真伪。
...
实机验证记录见 [`tests/forward-validation.md`](tests/forward-validation.md)。验证只覆盖表中列出的路径，不代表所有版本和安装方式都已覆盖。
...
- 初始化不会覆盖已有工作文件或交付文件
- 验证器能发现占位符、额外文件、原始材料副本和本机路径
- 普通、材料不足、材料冲突和敏感信息四类行为夹具已经建立

这不表示 Agent 已经自动证明成品中的事实正确。语义审核和最终放行仍由人完成。
...
- 尚未覆盖：Claude Code 的敏感资料前向用例；为避免把测试中的个人信息样式发送到外部服务，本次未执行
